...
class ContainerMonitor:
    ...
    def save_results(self):
        ...
        # Save raw data
        raw_data_file = RESULTS_DIR / "raw_data" / f"performance_data_{self.timestamp}.json"
    ...
    def create_visualizations(self):
        ...
        viz_file = RESULTS_DIR / "visualizations" / f"system_usage_{self.timestamp}.png"
        ...
        if gpu_data_available:
            ...
            num_gpus = max([len(d["gpu"]["devices"]) for d in self.data if "gpu" in d and "devices" in d["gpu"]])
            ...
            gpu_viz_file = RESULTS_DIR / "visualizations" / f"gpu_usage_{self.timestamp}.png"
            ...
            plt.title('GPU Memory (VRAM) Usage During Parallel Container Execution')
            plt.xlabel('Time (seconds)')
            ...
            vram_viz_file = RESULTS_DIR / "visualizations" / f"vram_usage_{self.timestamp}.png"
        ...
        if container_names:
            ...
            container_viz_file = RESULTS_DIR / "visualizations" / f"container_cpu_{self.timestamp}.png"
            ...
            container_mem_viz_file = RESULTS_DIR / "visualizations" / f"container_memory_{self.timestamp}.png"
...
def run_container(model_name, prompt, container_name=None):
    ...
    if not container_name:
        container_name = f"ollama-test-{model_name.split(':')[0]}-{time.time()}"
...
def run_parallel_test(models=None, duration=60):
    ...
    summary_file = RESULTS_DIR / "raw_data" / f"summary_{monitor.timestamp}.txt"
    with open(summary_file, "w") as f:
        ...
        # Calculate averages
        cpu_avg = sum(d["system"]["cpu_percent"] for d in monitor.data) / len(monitor.data) if monitor.data else 0
        mem_avg = sum(d["system"]["memory_percent"] for d in monitor.data) / len(monitor.data) if monitor.data else 0
