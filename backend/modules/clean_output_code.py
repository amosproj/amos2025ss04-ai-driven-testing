...
class CleanOutputCode(ModuleBase):
    ...
    def _create_fixing_prompt(
        ...
        analysis_error_info = self.extract_analysis_error_info(
    ...
    def _check_if_needs_fixing(
        self, prompt_data: PromptData, response_data: ResponseData
    ) -> bool:
        # Check for execution errors
        execution_errors = (
            self._extract_error_info_from_execution(response_data) is not None
        )

        # Check for analysis errors
        analysis_errors = (
            self.extract_analysis_error_info(prompt_data, response_data)
            is not None
        )
        if analysis_errors:
    ...
    def extract_analysis_error_info(
        ...
        try:
            ...
            if self._is_similar(test_code, prompt_code, threshold=0.7):
                errors.append(
                    "The generated test code appears to be mostly a copy of the source code. Please generate proper test cases instead of copying the original code."
                )
            ...
            copied_methods = [
                method for method in prompt_methods if method in test_methods
            ]
            if copied_methods:
                ...
                errors.append(
                    f"The test code contains {len(copied_methods)} method signatures copied directly from the source code: {methods_list}. Test methods should have unique names, typically starting with 'test_'."
                )
            ...
            if "def test_" in test_code.lower():
                ...
                if self._has_empty_test_methods(test_code):
                    errors.append(
                        "The test code contains empty or placeholder test methods. Each test method should include proper test logic and assertions."
                    )
            else:
                # No test methods at all
                if (
                    "unittest" in test_code.lower()
                    or "class Test" in test_code
                ):
                    errors.append(
                        "The test code defines a test class but doesn't contain any test methods (methods starting with 'test_')."
                    )

            # Format and return the errors
            if errors:
                return (
                    "The following issues were detected in the generated test code:\n- "
                    + "\n- ".join(errors)
                )
    ...
    def _is_similar(
        self, content: str, reference: str, threshold: float = 0.7
    ) -> bool:
        ...
        similarity = len(matching_lines) / max(
            len(content_lines), len(reference_lines)
        )
