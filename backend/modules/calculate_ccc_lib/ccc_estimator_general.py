...
class CCCEstimator:
    ...
    def __init__(self):
        ...
        self.control_keywords = {
            ...
            "switch": 1,
            "case": 1,
            "default": 1,  # switch/case structures
            "try": 1,
            "catch": 1,
            "except": 1,
        ...
        self.function_def_pattern = re.compile(
            r"\b(?:func|function|def|public\s+static|private\s+static|void|int|string|bool|var|let|const|class\s+\w+\s*:\s*\w+|[a-zA-Z_][a-zA-Z0-9_]*)\s+([a-zA-Z_][a-zA-Z0-9_]*)\s*\(",
            re.IGNORECASE,
        )
    ...
    def _get_generic_tokens(self, line):
        ...
        tokens = re.findall(
            r"\b\w+\b|\S+", line
        )  # Simpler, catches sequences of non-whitespace or word chars
    ...
    def _calculate_Wn_general(
        self, current_indentation, class_indentations, current_brace_level
    ):
        """
        ...
        for (
            ...
            _,
            _,
        ) in class_indentations:  # Adjusted to unpack the tuple
        ...
        level = 0
        for i, (indent, _, _) in enumerate(
            class_indentations
        ):  # Adjusted to unpack the tuple
    ...
    def _calculate_Wrc_general(self, line, method_name_stack):
        ...
        if method_name_stack:
            ...
            if re.search(
                r"\b" + re.escape(current_method) + r"\s*\(",
                line,
                re.IGNORECASE,
            ):
                weight += 1
    ...
    def _calculate_Wad_general(self, line):
        ...
        if match_collection:
            ...
            if (
            ...
            ):  # Avoid counting '*' as an element for 'new type[*]'
                ...
                elements_count = (
                    0  # Empty collection or direct size declaration
                )
        ...
        try:
            with open(file_path, "r", encoding="utf-8", errors="ignore") as f:
                for line_num, line in enumerate(f, 1):
                    ...
                    type_declaration_match = re.search(
                        r"\b(?:class|struct|interface|enum)\s+([a-zA-Z_][a-zA-Z0-9_]*)\b",
                        stripped_line,
                        re.IGNORECASE,
                    )
                    if type_declaration_match:
                        ...
                        while class_indentations and (
                            class_indentations[-1][0] >= current_indentation
                            or (
                                current_brace_level > 0
                                and current_brace_level
                                <= class_indentations[-1][2]
                            )
                        ):  # New brace level check
                            class_indentations.pop()
                        class_indentations.append(
                            (
                                current_indentation,
                                type_name,
                                current_brace_level,
                        ...
                        while class_indentations and (
                            current_indentation < class_indentations[-1][0]
                            or (
                                current_brace_level < class_indentations[-1][2]
                                and class_indentations[-1][2] > 0
                            )
                        ):
                            class_indentations.pop()

                    # Method/Function tracking
                    method_name_match = self.function_def_pattern.search(
                        stripped_line
                    )
                    if method_name_match:
                        ...
                        while method_name_stack and (
                            ...
                            and (
                                current_brace_level < method_name_stack[-1][1]
                                if len(method_name_stack[-1]) > 1
                                else 0
                            )
                        ):
                            method_name_stack.pop()
                        method_name_stack.append(
                            (method_name, current_brace_level)
                        )  # Store method name and its brace level
                    ...
                    W_n = self._calculate_Wn_general(
                        current_indentation,
                        class_indentations,
                        current_brace_level,
                    )
                    ...
                    W_cc = self._calculate_Wcc_general(
                        line, current_control_weight
                    )
                    W_io = self._calculate_Wio_general(line)

                    multiplier = max(1, W_c + W_n)
                    ccc_line = (
                        (S_k * multiplier) + W_tc + W_rc + W_ad + W_cc + W_io
                    )
