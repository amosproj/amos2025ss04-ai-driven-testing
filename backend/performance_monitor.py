...
class PerformanceMonitor:
    ...
    def get_gpu_metrics(self):
        ...
        if not self.has_nvidia_gpu:
            return {"error": "No NVIDIA GPU detected or nvidia-smi not available"}
        
        try:
            # Get GPU utilization
            gpu_util_output = subprocess.check_output(
                ['nvidia-smi', '--query-gpu=utilization.gpu,utilization.memory,memory.total,memory.used,memory.free', 
    ...
    def calculate_cpu_percent(self, stats):
        ...
        try:
            ...
            if system_delta > 0 and cpu_delta > 0:
                return round((cpu_delta / system_delta) * len(stats["cpu_stats"]["cpu_usage"]["percpu_usage"]) * 100, 2)
            return 0.0
        ...
        try:
            ...
            for container in self.docker_client.containers.list():
                ...
                container_stats[container.name] = {
                    ...
                    "memory_usage_mb": round(stats["memory_stats"].get("usage", 0) / (1024**2), 2),
                    "memory_limit_mb": round(stats["memory_stats"].get("limit", 0) / (1024**2), 2),
                ...
                if self.has_nvidia_gpu:
                    try:
                        ...
                        inspect = self.docker_client.api.inspect_container(container.id)
                        if "NvidiaRuntime" in str(inspect) or "nvidia" in str(inspect):
    ...
    def save_performance_metrics(self, metrics, filename="docker_performance.json"):
    ...
    if "gpu" in baseline and "gpu" in post_execution:
        ...
        if "devices" in baseline["gpu"] and "devices" in post_execution["gpu"]:
            ...
            if baseline["gpu"]["devices"] and post_execution["gpu"]["devices"]:
                gpu_deltas = []
                for i in range(min(len(baseline["gpu"]["devices"]), len(post_execution["gpu"]["devices"]))):
                    before_device = baseline["gpu"]["devices"][i]
                    after_device = post_execution["gpu"]["devices"][i]
                    gpu_deltas.append({
                        "index": i,
                        "gpu_util_delta": after_device["gpu_util_percent"] - before_device["gpu_util_percent"],
                        "mem_used_delta_mb": after_device["mem_used_mb"] - before_device["mem_used_mb"],
                        "mem_used_percent_delta": after_device["mem_used_percent"] - before_device["mem_used_percent"]
                    })
