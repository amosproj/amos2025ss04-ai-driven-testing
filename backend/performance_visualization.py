...
class PerformanceVisualizer:
    ...
    def __init__(self, output_dir=None):
        ...
        if output_dir:
            ...
            self.output_dir = Path(os.path.dirname(os.path.abspath(__file__))) / "performance_results"
    ...
    def _create_time_series_visualizations(self, snapshots):
        ...
        if gpu_data_available:
            ...
            num_gpus = max([len(d["gpu"]["devices"]) for d in snapshots if "gpu" in d and "devices" in d["gpu"]])
            ...
            for gpu_idx in range(num_gpus):
                ...
                for i, d in enumerate(snapshots):
                    if ("gpu" in d and "devices" in d["gpu"] and 
                        len(d["gpu"]["devices"]) > gpu_idx):
                        gpu_util_values.append(d["gpu"]["devices"][gpu_idx]["gpu_util_percent"])
                        gpu_times.append(relative_times[i])
                
                if gpu_util_values:
                    plt.plot(gpu_times, gpu_util_values, '-', 
            ...
            for gpu_idx in range(num_gpus):
                ...
                for i, d in enumerate(snapshots):
                    if ("gpu" in d and "devices" in d["gpu"] and 
                        len(d["gpu"]["devices"]) > gpu_idx):
                        vram_usage_values.append(d["gpu"]["devices"][gpu_idx]["mem_used_percent"])
                        vram_times.append(relative_times[i])
                
                if vram_usage_values:
                    plt.plot(vram_times, vram_usage_values, '-', 
        ...
        if container_names:
            container_data = {name: {"cpu": [], "memory": [], "times": []} for name in container_names}
            
            for i, d in enumerate(snapshots):
                for name in container_names:
                    if name in d["containers"]:
                        container_data[name]["cpu"].append(d["containers"][name]["cpu_percent"])
                        container_data[name]["memory"].append(d["containers"][name]["memory_usage_mb"])
            ...
            for name, data in container_data.items():
                if data["times"]:  # Only plot if we have data
                    plt.plot(data["times"], data["cpu"], '-', label=f'{name} CPU (%)')
            ...
            container_cpu_path = self.viz_dir / f"container_cpu_{self.timestamp}.png"
            ...
            for name, data in container_data.items():
                if data["times"]:  # Only plot if we have data
                    plt.plot(data["times"], data["memory"], '-', label=f'{name} Memory (MB)')
            ...
            container_mem_path = self.viz_dir / f"container_memory_{self.timestamp}.png"
    ...
    def _create_before_after_visualizations(self, perf_data):
        ...
        if not ("system_metrics" in perf_data and "before" in perf_data["system_metrics"] and "after" in perf_data["system_metrics"]):
        ...
        plt.tight_layout()
        system_comparison_path = self.viz_dir / f"system_comparison_{self.timestamp}.png"
        plt.savefig(system_comparison_path)
        plt.close()
        viz_paths["system_comparison"] = str(system_comparison_path)
        
        # GPU comparison if available
        if "gpu_metrics" in perf_data and "before" in perf_data["gpu_metrics"] and "after" in perf_data["gpu_metrics"]:
            if ("devices" in perf_data["gpu_metrics"]["before"] and 
                "devices" in perf_data["gpu_metrics"]["after"] and
                perf_data["gpu_metrics"]["before"]["devices"] and 
                perf_data["gpu_metrics"]["after"]["devices"]):
                
                # GPU utilization comparison for each GPU
                num_gpus = min(len(perf_data["gpu_metrics"]["before"]["devices"]), 
                               len(perf_data["gpu_metrics"]["after"]["devices"]))
                
                fig, axes = plt.subplots(num_gpus, 2, figsize=(12, num_gpus * 4))
                if num_gpus == 1:
                    axes = [axes]  # Make 2D for consistent indexing
                
                for i in range(num_gpus):
                    before_gpu = perf_data["gpu_metrics"]["before"]["devices"][i]
                    after_gpu = perf_data["gpu_metrics"]["after"]["devices"][i]
                    
                    # GPU utilization
                    gpu_labels = ['Before', 'After']
                    gpu_values = [before_gpu["gpu_util_percent"], after_gpu["gpu_util_percent"]]
                    axes[i][0].bar(gpu_labels, gpu_values, color=['blue', 'red'])
                    axes[i][0].set_ylabel('GPU Utilization (%)')
                    axes[i][0].set_title(f'GPU {i} Utilization Before vs After')
                    axes[i][0].grid(axis='y', linestyle='--', alpha=0.7)
                    
                    # VRAM usage
                    vram_labels = ['Before', 'After']
                    vram_values = [before_gpu["mem_used_percent"], after_gpu["mem_used_percent"]]
                    axes[i][1].bar(vram_labels, vram_values, color=['blue', 'red'])
                ...
                plt.tight_layout()
                gpu_comparison_path = self.viz_dir / f"gpu_comparison_{self.timestamp}.png"
    ...
    def _generate_time_series_report(self, snapshots):
        ...
        report.append(f"Test Duration: {snapshots[-1]['timestamp'] - snapshots[0]['timestamp']:.2f} seconds")
        report.append(f"Number of Samples: {len(snapshots)}\n")
        
        # System metrics
        cpu_avg = sum(d["system"]["cpu_percent"] for d in snapshots) / len(snapshots)
        cpu_max = max(d["system"]["cpu_percent"] for d in snapshots)
        mem_avg = sum(d["system"]["memory_percent"] for d in snapshots) / len(snapshots)
            ...
            for gpu_idx in range(num_gpus):
                ...
                for d in snapshots:
                    if ("gpu" in d and "devices" in d["gpu"] and 
                        len(d["gpu"]["devices"]) > gpu_idx):
                        gpu_util_values.append(d["gpu"]["devices"][gpu_idx]["gpu_util_percent"])
                        vram_values.append(d["gpu"]["devices"][gpu_idx]["mem_used_percent"])
                ...
                if gpu_util_values:
                    ...
                    report.append(f"    Utilization Average: {gpu_util_avg:.2f}%")
        ...
        if container_names:
            ...
            for name in container_names:
                ...
                if container_data:
                    cpu_avg = sum(c["cpu_percent"] for c in container_data) / len(container_data)
                    cpu_max = max(c["cpu_percent"] for c in container_data)
                    mem_avg = sum(c["memory_usage_mb"] for c in container_data) / len(container_data)
    ...
    def _generate_before_after_report(self, perf_data):
        ...
        if "system_metrics" in perf_data:
            report.append("SYSTEM RESOURCE USAGE:")
            report.append(f"  CPU Before: {perf_data['system_metrics']['before']['cpu_percent']:.2f}%")
            report.append(f"  CPU After: {perf_data['system_metrics']['after']['cpu_percent']:.2f}%")
            report.append(f"  CPU Delta: {perf_data['system_metrics']['cpu_delta']:.2f}%")
            report.append(f"  Memory Before: {perf_data['system_metrics']['before']['memory_percent']:.2f}%")
            report.append(f"  Memory After: {perf_data['system_metrics']['after']['memory_percent']:.2f}%")
            report.append(f"  Memory Delta: {perf_data['system_metrics']['after']['memory_percent'] - perf_data['system_metrics']['before']['memory_percent']:.2f}%")
        ...
        if "gpu_metrics" in perf_data and "deltas" in perf_data["gpu_metrics"]:
            ...
            for i, gpu in enumerate(perf_data["gpu_metrics"]["deltas"]):
                ...
                report.append(f"    Utilization Delta: {gpu['gpu_util_delta']:.2f}%")
                report.append(f"    VRAM Usage Delta: {gpu['mem_used_delta_mb']:.2f} MB")
                report.append(f"    VRAM Usage % Delta: {gpu['mem_used_percent_delta']:.2f}%")
        ...
        if "container_metrics" in perf_data:
            ...
            if before_containers or after_containers:
                ...
                container_names = set(before_containers.keys()) | set(after_containers.keys())
                ...
                for name in container_names:
                    ...
                    if name in before_containers and name in after_containers:
                        ...
                        report.append(f"    CPU Delta: {cpu_after - cpu_before:.2f}%")
                        report.append(f"    Memory Before: {mem_before:.2f} MB")
                        report.append(f"    Memory After: {mem_after:.2f} MB")
                        report.append(f"    Memory Delta: {mem_after - mem_before:.2f} MB")
                    elif name in before_containers:
                        report.append("    Container only present before test")
                        report.append(f"    CPU: {before_containers[name]['cpu_percent']:.2f}%")
                        report.append(f"    Memory: {before_containers[name]['memory_usage_mb']:.2f} MB")
                    else:
                        report.append("    Container only present after test")
                        report.append(f"    CPU: {after_containers[name]['cpu_percent']:.2f}%")
                        report.append(f"    Memory: {after_containers[name]['memory_usage_mb']:.2f} MB")
