...
logging.basicConfig(
    ...
    handlers=[logging.StreamHandler(), logging.FileHandler('docker_runner.log')]
)
...
def ensure_no_leftover_containers():
    ...
    try:
        ...
        cmd = ["docker", "ps", "-a", "--filter", "name=sequential-runner-", "--format", "{{.ID}}"]
        ...
        for container_id in container_ids:
            if container_id:
                logging.warning(f"Found leftover container {container_id}, removing it")
...
def run_container(container_name, input_data):
    ...
    try:
        ...
        subprocess.run(["docker", "pull", container_name], check=True, capture_output=True)
        ...
        logging.info(f"Starting container: {container_name} as {unique_container_name}")
        container_process = subprocess.run(
            ["docker", "run", "--name", unique_container_name, "--rm", "-i", container_name, 
             "sh", "-c", "cat && echo 'Processed input'"],
        ...
        verify_cmd = ["docker", "ps", "-a", "--filter", f"name={unique_container_name}", "--format", "{{.ID}}"]
        ...
        if verify_result.stdout.strip():
            ...
            logging.warning(f"Container {unique_container_name} still running, force removing")
            subprocess.run(["docker", "rm", "-f", container_id], check=True, capture_output=True)
        ...
        try:
            ...
            if verify_result.stdout.strip():
                container_id = verify_result.stdout.strip()
                logging.warning(f"Removing failed container {unique_container_name}")
                subprocess.run(["docker", "rm", "-f", container_id], check=True, capture_output=True)
        except Exception as cleanup_error:
            logging.error(f"Error during container cleanup: {str(cleanup_error)}")
        
        return error_msg
    except Exception as e:
        error_msg = f"Unexpected error with container {container_name}: {str(e)}"
        logging.error(error_msg)
...
def run_container_sequence():
    """
    Main function to run containers sequentially
    """
    results = []
    ...
    try:
        ...
        logging.error("Docker is not available. Please install Docker and ensure it's running.")
        return False
    ...
    for i, (container, input_data) in enumerate(CONTAINERS):
        ...
        logging.info(f"Processing container {i+1}/{len(CONTAINERS)}: {container}")
    ...
    try:
        cmd = ["docker", "ps", "--filter", "name=sequential-runner-", "--format", "{{.ID}}"]
        result = subprocess.run(cmd, check=True, capture_output=True, text=True)
        if result.stdout.strip():
            logging.error(f"WARNING: There are still containers running: {result.stdout.strip()}")
        else:
            logging.info("Verified no containers are running - cleanup successful")
    except Exception as e:
...
def input_listener():
    """
    Thread function to listen for 'q' key to quit
    """
    global running
...
def cleanup():
    """
    Function to ensure cleanup on program exit
    """
...
def main():
    """
    Main function with continuous execution loop
    """
    global running
    ...
    print("Container runner started. Containers will run in sequence continuously.")
    ...
    try:
        ...
        while running:
            ...
            if running:
                logging.info(f"Waiting 10 seconds before next sequence...")
