...
class Testdetect_cycle:
    ...
    def main(self):
        ...
        for no_cycle, name in zip([None, True, False, True], ["No cycle in acyclic graph", "Single node is considered a cycle", "Two separate cycles exist", "Large cycle with multiple nodes exists"]):
            self.setup(lambda: Node(no_cycle))
            result = self.detect_cycle(node)
            assert not no_cycle and (result if no_cycle is None else result), f"{name} should be {result}"
