...
class TestgetFactors(unittest.TestCase):
    def test_get_factors(self, n):
        if n == 1:
            ...
            result = get_factors(n)
            expected = [i for i in range(2, int(n ** 0.5) + 1) if (n % i == 0 and i != n) or (n // i == 1 and i < n) or (n % i == 0)]
            self.assertEqual(result, expected)
...
class TestgetFactors(unittest.TestCase):
    def test_get_factors(self, n):
        if n == 1:
            ...
            result = get_factors(n)
            expected = [i for i in range(2, int(n ** 0.5) + 1) if (i != n and n % i == 0) or (i < n and n // i == 1)]
            self.assertEqual(result, expected)
