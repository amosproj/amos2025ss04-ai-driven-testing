...
class TestKHAPSort(unittest.TestCase):
    ...
    def test_kheapsort(self):
        ...
        arr = [1, 4, 2, 8, 9]
        expected_sorted = sorted(arr[:k]) + list(kheapq.heappop(heap) for _ in range(len(arr)-len(expected_sorted)))
    ...
    def test_kheapsort_with_empty(self):
        ...
        arr = []
        with self.assertRaises(TypeError):  # Kheapsort should raise TypeError for invalid input
