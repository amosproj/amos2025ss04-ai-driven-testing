...
def topological_ordering(nodes):
    ...
    while True:
        ...
        for node in list(ordered_nodes):  # Make a copy of the list since we're modifying it
            for nextnode in node.outgoing_nodes:
                if set([n.name for n in ordered_nodes]).issuperset(nextnode.incoming_nodes) and not any(n == nextnode for n in ordered_nodes):
                    moved_in_this_cycle = True
                    break

        # If no nodes were moved, we are done.
        if not moved_in_this_cycle:
            return [node for node in sorted(ordered_nodes, key=lambda x: x.name)]
...
class TestTopologicalOrdering(unittest.TestCase):
    ...
    def test_ordering_single_node(self):
        ...
        for n in sorted(result, key=lambda x: x.name):  # Sort to match expected order
    ...
    def test_ordering_two_nodes_with_dependency_a_on_b(self):
        ...
        result_names = [n.name for n in sorted(ordered_result, key=lambda x: x.name)]
    ...
    def test_topological_sort_cycle(self):
        ...
        nodes = create_example_graph()
        for node in sorted(nodes):  # Sort to make sure we check with consistent starting order.
            if "B" not in [n.name for n in ordered_nodes]:
                raise Exception("Cycle detected. Topological sorting is impossible.")
