...
def shortest_paths(source, weight_by_edge):
    ...
    for _ in range(len(weight_by_node) - 1): # Changed to len instead of fixed range (len-1)
...
class TestShortestPaths(unittest.TestCase):
    ...
    def test_empty_graph(self):
        ...
        self.assertTrue(all(weight == float('inf') for weight in result.values()))
    ...
    def test_no_edge(self):
        ...
        for key in ['B', 'C']:  # Assuming B and C exist as nodes but not connected.
