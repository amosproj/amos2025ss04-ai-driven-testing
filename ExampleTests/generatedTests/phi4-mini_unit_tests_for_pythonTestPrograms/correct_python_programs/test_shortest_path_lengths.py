        ...
        if len(edges_to_update[k]) > 0:
            u_v_pairs = [
                (
                    u + "->" + v,
                    (
                        paths_by_vertex[u + "->" + k][0]
                        + paths_by_vertex[edges_to_update[v][-2]][-1],
                        edges_to_update[v][-1],
                    ),
                )
                for u in range(k + 1)
                for v in edges_to_update[k]
            ]
            updated_paths.update(u_v_pairs)

    return {
        k: min(v, default=path_length())
        if isinstance(v, list)
        else paths_by_vertex[k]
        for k, v in updated_paths.items()
    }
...
class TestShortestPathLengths(unittest.TestCase):
    ...
    def test_complex_paths(self):
        ...
        self.assertEqual(
            ...
            min(
                10,
                paths[(0, 2)][-1] + paths[(2, 3)][-1],
                keys=(paths[(0, 2)].keys())[0],
            ),
        )
