...
class TestReverseLinkedList(unittest.TestCase):
    def setUp(self):
        ...
        while True:
            ...
            next_value, successor_ref = yield_current_and_successor(
                current_node
            )

            new_next_node = Node(
                next_value, successor=successor_ref or self.list_head_b
            )  # Dummy code to replicate linked list structure
    ...
    def setUpClass():
        ...
        for i in range(
            ...
            next_value, successor_ref = yield_next(
                current_head
            )  # Dummy code to replicate linked list structure
...
class TestReverseLinkedList(unittest.TestCase):
    def test_reverse_functionality(self):
        ...
        for i in range(0, len(expected_result_values)):
            ...
            original_value, successor_ref = yield_current_and_successor(
                self.test_node
            )
            ...
            while True:
                current_test_val, next_succesor_ref = yield_next(
                    reversed_linked_list_head
                )  # Dummy code to replicate linked list structure
                self.assertEqual(
                    current_test_val.value, expected_result_values[i]
                )

                if (
                    not next_succesor_ref
                    or len(expected_result_values) - i - 1 == 0
                ):
                    break
