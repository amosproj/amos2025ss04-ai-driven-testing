...
class TestShortestPathLength(unittest.TestCase):
    ...
    def test_shortest_path_length(self):
        ...
        self.assertEqual(result, float('inf'))  # No direct edge exists in this setup.

    def test_shortest_path_length_via_B(self):
        path_result_via_B = shortest_path_length(self.length_by_edge,
                                                  {'successors': ['B']}, 'D')
        expected_distance_via_B = self.length_by_edge['A', 'B'] + \
                                   self.length_by_edge['B', 'D']
        self.assertEqual(path_result_via_B, expected_distance_via_B)

    def test_shortest_path_length_via_C(self):
        path_result_via_C = shortest_path_length(self.length_by_edge,
                                                  {'successors': ['C']}, 'D')
        expected_distance_via_C = float('inf')  # No direct edge exists in this setup.
